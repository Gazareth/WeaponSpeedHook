...
const int OPCODEBASE        = 0x5000; //TODO -- GET A PROPER OPCODE!!!111oneee
...
extern "C" bool _declspec(dllexport) OBSEPlugin_Query(const OBSEInterface* obse, PluginInfo* info)
{
    ...
    _gLogFile = new HTMLTarget( obse->isEditor  ? "Data\\obse\\plugins\\" TO_STR(SOLUTIONNAME) "\\CS.log.html" : "Data\\obse\\plugins\\" TO_STR(SOLUTIONNAME) "\\Game.log.html");
    gLog.AttachTarget(*_gLogFile);     

	// fill out plugin info structure
	info->infoVersion = PluginInfo::kInfoVersion;   // info structure version
	info->name = TO_STR(SOLUTIONNAME);                      // plugin name
	info->version = PLUGINVERSION;                  // plugin version
    _MESSAGE("OBSE Query (CSMode = %i), v%08X",obse->isEditor,PLUGINVERSION);

    // check obse version
    g_obseIntfc = obse;
    if(obse->obseVersion+1 < OBSE_VERSION_INTEGER)
	{
		_FATALERROR("OBSE version too old (got %u expected at least %u)", obse->obseVersion, OBSE_VERSION_INTEGER);
		return false;
	}
    ...
    const char* modulename = obse->isEditor ? "Data\\obse\\plugins\\" TO_STR(SOLUTIONNAME) "\\Submodule.CS.dll" 
                                            : "Data\\obse\\plugins\\" TO_STR(SOLUTIONNAME) "\\Submodule.Game.dll";
...
{// called when plugin is loaded into process memory, before obse takes over
	...
	case DLL_PROCESS_ATTACH:
		_MESSAGE("Attaching Loader ...");
    case DLL_PROCESS_DETACH:    // dll unloaded 
		_MESSAGE("Detaching Loader ...");
	...
	default:
		if (_gLogFile){
			//_MESSAGE("DLL MESSAGE REASON -- %X", dwReason);
		}
		break;
    }   
	return true;
